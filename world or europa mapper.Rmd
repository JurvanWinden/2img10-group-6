...
infection_corona_world <- infection_corona_world1 %>% filter(day == '15', month == '4',)
...
#begin world
infection_corona_world_dist = dist(infection_corona_world[,13:14]) #, method = "manhattan"
#end world
...
#option 2
 ...
 for (i in 1:length(infection_corona_world$deaths ) ) {
   infection_corona_world$casesRatio[i] <-
    if (infection_corona_world$cases[i] > 0){
      
      round(infection_corona_world$popData2018[i] / infection_corona_world$cases[i])
...
#begin world mapper
infection_corona_world_mapper <- mapper(dist_object = infection_corona_world_dist,
                                filter_values = infection_corona_world$sumDeaths,
                                num_intervals = 20,
                                percent_overlap = 80,
...
#begin world
value.mean.vertex <- rep(0,infection_corona_world_mapper2d$num_vertices)
for (i in 1:infection_corona_world_mapper2d$num_vertices){
  points.in.vertex <- infection_corona_world_mapper2d$points_in_vertex[[i]]
  # value.mean.vertex[i] <-mean((infection_corona_world$deaths[points.in.vertex]))
  value.mean.vertex[i] <- round(mean((infection_corona_world$cases[points.in.vertex])))
}
# value.mean.vertex
#end world
...
vertex.size <- rep(0,infection_corona_world_mapper2d$num_vertices)
for (i in 1:infection_corona_world_mapper2d$num_vertices){
  points.in.vertex <- infection_corona_world_mapper2d$points_in_vertex[[i]]
  vertex.size[i] <- length((infection_corona_world_mapper2d$points_in_vertex[[i]]))
}
vertex.size
```
...
V(infection_corona_world_graph2)$color <- value.mean.vertex.grey
V(infection_corona_world_graph2)$size <- vertex.size
plot(infection_corona_world_graph2,main ="Mapper Graph")
...
```{r}
library(networkD3)
MapperNodes <- mapperVertices(infection_corona_world_mapper2d, 1:1000 )
MapperLinks <- mapperEdges(infection_corona_world_mapper2d)
...
for (i in (1: nrow(MapperNodes))){
  ...
  for (j in (2 : (length(split[[1]])) ) ) {
    country <-  infection_corona_world$countriesAndTerritories[as.integer(gsub(",","",split[[1]][j]))]
